...
var _BranchText = ""; //暂时不用了
var _OperationFlag = "";  //  create  rename remove
var _SuperiorID = ""; //上级ID
var _CurrentID = ""; //当前的选择的ID

var _CurrentIDF = "";
var _CurrentText = ""; //选择的文本
	...
	$.jstree.core.prototype = {
		...
		init : function (el, options) {
			...
			this._data.core.original_container_html
				...
				.remove();
			this.element.html("<"+"ul class='jstree-container-ul jstree-children' role='group'><"+"li id='j"+this._id+"_loading' class='jstree-initial-node jstree-loading jstree-leaf jstree-last' role='tree-item'><i class='jstree-icon '></i><"+"a class='jstree-anchor' href='#'><i class='jstree-icon jstree-themeicon-hidden'></i>" + this.get_string("Loading ...") + "</a></li></ul>");
			...
			*/
			...
			h2 = $("<"+"input />", {
						...
						"blur": $.proxy(function (e) {
							...
							_BranchText = v;
						    //alert(_BranchText); //f123456   rename remove
							switch (_OperationFlag) {
							    case "create":
							        create(_BranchText, _SuperiorID); break;
							    case "rename":
							        rename(_BranchText,_CurrentID); break;
							    case "remove":
							        remove(_CurrentID); break;
							}
							if(v === "") { v = t; }
							h1.remove();
							s.replaceWith(a);
			...
			return {
				"create" : {
					...
					"action": function (data) {
					    ...
					    //console.log(data.reference.parents("li").attr("id"));  //获取所属上级ID f789
				...
				"remove" : {
					...
					"action"			: function (data) {
						var inst = $.jstree.reference(data.reference),
							obj = inst.get_node(data.reference);
					    //_CurrentID = data.reference.parents("li").attr("id");
						//_OperationFlag = "remove";
						//remove(_CurrentID);
			...
			vakata_context.element
				.on("mouseenter", "li", function (e) {
					...
					vakata_context.element.find("." +
					    ...
					    "" +
					    "" +
					    "-hover").removeClass("vakata-context-hover").end();
	...
	(function ($) {
		...
		$.vakata.dnd = {
			...
			start: function (e, data, html) {
			    _CurrentIDF = e.target.id.split("_")[0];

			    _CurrentText = e.target.outerText;
			...
			stop: function (e) {
			    ...
			    //alert(_CurrentText + "-->" + e.target.outerText);
				...
				if(vakata_dnd.is_drag) {
				    ...
				    //alert(_CurrentIDF + "-" + e.target.id.split("_")[0]);
				    //_CurrentIDF当前Id  e.target.id.split("_")[0]上级部门
				    if (_CurrentIDF != "" && _CurrentIDF != e.target.id.split("_")[0]) {
				        //方法实现；
				      
				 

				    }
				//    editLevelandSupervisorDept (_CurrentIDF, _CurrentText, e.target.id.split("_")[0], e.target.outerText);
