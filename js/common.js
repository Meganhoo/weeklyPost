...
//获取选中节点
//以及选中节点的父节点Id
function getJsTreeCheckedNodesIdArray(treeviewId, args) {
    if (args == undefined) {
        args = new Object();
    }
...
function getJsTreeCheckedNodesArray(treeviewId, args) {
    if (args == undefined) {
        args = new Object();
    }
...
function getJsTreeSelectedNodesIdArray(treeviewId, args) {
    ...
    var selectedNodeList = $('#' + treeviewId).jstree().get_selected(true);
    //   alert(selectedNodeList.length);
    var result = new Array();
    var flag;
    for (var i = 0; i < selectedNodeList.length; i++) {
